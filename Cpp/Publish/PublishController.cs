...
namespace SE.Hecate.Cpp
{
    ...
    public class PublishController : ProcessorUnit
    {
        ...
        {
            get { return (UInt32)ProcessorFamilies.CppPublish; }
        }
        ...
        private async static Task<int> Process(IEnumerable<BuildModule> modules)
        {
            ...
            try
            {
                foreach (BuildModule module in modules.OrderByDescending(x => x, CppModuleComparer.Default))
                {
                    if (!module.IsPackage)
                    {
                        CppModule cpp;
                        ...
                        if (process)
                        {
                            Application.Log(SeverityFlags.Full, "Preparing package creation '{0}' (cpp)", module.Name);
                        }
                        else continue;
                        #endregion
                        ...
                        tasks.Add(Task.Run(() => CreatePackageMeta(module, cpp, package)).ContinueWith((task) =>
                        {
                            if (task.Status == TaskStatus.RanToCompletion)
                            {
                                ...
                                Application.Log(SeverityFlags.Minimal, "Create package {0} (cpp)", package.FriendlyName(false));
                            }
                            else Application.Error(SeverityFlags.None, "Package creation failed {0} (cpp)", package.FriendlyName(false));
        ...
        private static void CreatePackageMeta(BuildModule module, CppModule cpp, PackageMeta package)
        {
