...
#pragma warning restore IDE1006 // Naming Styles
        ...
        private void InitializeWriter()
        {
            ...
            _xmlTextWriter.WriteStartElement(ResHeaderStr);
            {
                ...
                _xmlTextWriter.WriteStartElement(ValueStr);
                {
                    _xmlTextWriter.WriteString(Utils.GetAssemblyQualifiedName(typeof(ResXResourceReader), _typeNameConverter));
                }
            ...
            _xmlTextWriter.WriteStartElement(ResHeaderStr);
            {
                ...
                _xmlTextWriter.WriteStartElement(ValueStr);
                {
                    _xmlTextWriter.WriteString(Utils.GetAssemblyQualifiedName(typeof(ResXResourceWriter), _typeNameConverter));
                }
        ...
        {
            ResXDataNode node; if ((node = value as ResXDataNode) != null)
            {
                AddResource(node);
            }
            else
        ...
        private void AddDataRow(string elementName, string name, object value)
        {
            ...
            if (value is string)
            {
                AddDataRow(elementName, name, value as string);
            }
            else if (value is byte[])
            {
                AddDataRow(elementName, name, value as byte[]);
            }
            else if (value is ResXFileRef)
            {
                ResXDataNode node = new ResXDataNode(name, value as ResXFileRef, _typeNameConverter);
        ...
        private void AddDataRow(string elementName, string name, string value)
        {
            ...
            string typeName =
                value == null
                    ? Utils.GetAssemblyQualifiedName(typeof(ResXNullRef), _typeNameConverter)
                    : null;
            ...
            if (!string.IsNullOrEmpty(type) && elementName == DataStr)
            {
                ...
                if (string.IsNullOrEmpty(assemblyName))
                {
                    try
                    {
                        ...
                        {
                            assemblyName = GetFullName(Utils.GetAssemblyQualifiedName(typeObject, _typeNameConverter));
        ...
        {
            string result = Utils.GetAssemblyQualifiedName(type, _typeNameConverter);
            return result;
        }
