    ...
    public sealed class ResXDataNode : ISerializable
    {
        ...
        internal ResXDataNode DeepClone()
        {
            return new ResXDataNode
            {
                ...
                _nodeInfo = _nodeInfo != null ? _nodeInfo.Clone() : default(DataNodeInfo),
                _name = _name,
                _comment = _comment,
                _typeName = _typeName,
                ...
                _value = _value,
                _fileRef = _fileRef != null ? _fileRef.Clone() : default(ResXFileRef),
        ...
        public ResXDataNode(string name, object value, Func<Type, string> typeNameConverter)
        {
            ...
            {
                throw new InvalidOperationException(valueType.FullName);
            }

            if (value != null)
            {
                _typeName = Utils.GetAssemblyQualifiedName(valueType, _typeNameConverter);
            }
        ...
        public ResXDataNode(string name, ResXFileRef fileRef, Func<Type, string> typeNameConverter)
        {
            ...
            else throw new ArgumentNullException("fileRef");
            _typeNameConverter = typeNameConverter;
        }
        ...
        private string FileRefFullPath
        {
            get
            {
                if (_fileRef != null)
                {
                    return _fileRef.FileName;
                }
                else return _fileRefFullPath;
            }
        }
        ...
        private string FileRefType
        {
            get
            {
                if (_fileRef != null)
                {
                    return _fileRef.TypeName;
                }
                else return _fileRefType;
            }
        }
        ...
        private string FileRefTextEncoding
        {
            get
            {
                if (_fileRef != null && _fileRef.TextFileEncoding != null)
                {
                    return _fileRef.TextFileEncoding.BodyName;
                }
                else return _fileRefTextEncoding;
            }
        }
        ...
        {
            CultureInfo ci; if ((ci = value as CultureInfo) != null)
            {
                // special-case CultureInfo, cannot use CultureInfoConverter for serialization
                nodeInfo.ValueData = ci.Name;
                nodeInfo.TypeName = Utils.GetAssemblyQualifiedName(typeof(CultureInfo), _typeNameConverter);
            }
            else
            {
                string str; if ((str = value as string) != null)
                {
                    nodeInfo.ValueData = str;
                }
                else
                {
                    byte[] bytes; if ((bytes = value as byte[]) != null)
                    {
                        nodeInfo.ValueData = ToBase64WrappedString(bytes);
                        nodeInfo.TypeName = Utils.GetAssemblyQualifiedName(typeof(byte[]), _typeNameConverter);
                    }
                    else
                    {
                        Type valueType = (value == null) ? typeof(object) : value.GetType();
                        ...
                        try
                        {
                            if (toString && fromString)
                            {
                                ...
                                nodeInfo.TypeName = Utils.GetAssemblyQualifiedName(valueType, _typeNameConverter);
                                return;
                            }
                        }
                        catch (Exception ex)
                        ...
                        if (value == null)
                        {
                            ...
                            nodeInfo.TypeName = Utils.GetAssemblyQualifiedName(typeof(ResXNullRef), _typeNameConverter);
                        }
                        else
                        {
        ...
        private object GenerateObjectFromDataNodeInfo(DataNodeInfo dataNodeInfo, ITypeResolutionService typeResolver)
        {
            ...
            string typeName =
                string.IsNullOrEmpty(dataNodeInfo.TypeName)
                    ? Utils.GetAssemblyQualifiedName(typeof(string), _typeNameConverter)
...
#pragma warning restore SYSLIB0011 // Type or member is obsolete
        ...
        internal DataNodeInfo GetDataNodeInfo()
        {
            ...
            if (shouldSerialize || FileRefFullPath != null)
            {
                ...
                if (FileRefFullPath != null)
                {
                    ...
                    _nodeInfo.TypeName = Utils.GetAssemblyQualifiedName(typeof(ResXFileRef), _typeNameConverter);
                }
                else
                {
        ...
        public Point GetNodePosition()
        {
            if (_nodeInfo != null)
            {
                return _nodeInfo.ReaderPosition;
            }
        ...
        public string GetValueTypeName(ITypeResolutionService typeResolver)
        {
            ...
            {
                return
                    _typeName == Utils.GetAssemblyQualifiedName(typeof(ResXNullRef), _typeNameConverter)
                        ? Utils.GetAssemblyQualifiedName(typeof(object), _typeNameConverter)
                        : _typeName;
            }
            ...
            else if (_nodeInfo != null)
            {
                ...
                // if typename == null, the default is just a string
                if (string.IsNullOrEmpty(result))
                {
                    ...
                    if (!string.IsNullOrEmpty(_nodeInfo.MimeType))
                    {
                        ...
                        catch (Exception ex)
                        {
                            ...
                            result = Utils.GetAssemblyQualifiedName(typeof(object), _typeNameConverter);
                        }

                        if (insideObject != null)
                        {
                            result = Utils.GetAssemblyQualifiedName(insideObject.GetType(), _typeNameConverter);
                        }
                    }
                    else
                    {
                        // no typename, no mimetype, we have a string...
                        result = Utils.GetAssemblyQualifiedName(typeof(string), _typeNameConverter);
                    }
                }
                else
                {
            ...
            if (objectType != null)
            {
                ...
                {
                    result = Utils.GetAssemblyQualifiedName(objectType, _typeNameConverter);
                }
            }
